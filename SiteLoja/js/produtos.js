...
const shirtImages = {
    ...
    johnmarston: {
        branca: "../produtos/CB/2 john marston red dead redemption camisa branca sem fundo.png",
        preta: "../produtos/CP/1 john marston red dead redemption camisa preta sem fundo.png"
    },
    luffy: {
...
function loadUserPreference() {
    ...
    if (savedColor && shirtImages[currentDesign][savedColor]) currentColor = savedColor;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".demo-shirt").forEach(shirt => {
        shirt.addEventListener("click", () => {
            const color = shirt.parentElement.id === "position-1" ? "branca" : "preta";
            changeColor(color);
            saveUserPreference();
        });
    });
...
window.onload = async function () {
    ...
    try {
        ...
        const nomeProcessado = produto.nome.toLowerCase().replace(/\s/g, "").replace(/[^a-z0-9]/g, "");
        ...
        document.getElementById("preco-produto").textContent = `R$ ${produto.preco.toFixed(2).replace('.', ',')}`;

        // Atualiza imagem inicial e demos
        if (shirtImages[currentDesign]) {
            document.getElementById("imagem-produto").src = shirtImages[currentDesign][currentColor];
            updateDemoImages();
        } else {
